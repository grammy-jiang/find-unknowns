"""Install/inspect/remove the find-unknowns skill for coding-agent platforms.

Architecture copied deliberately from socratic-method's installer (see
docs/design.md, Constraint 3 — copy, not dependency). Design rules:
...
- **Verify before claiming.** After writing, every file is read back from disk
  ...
  "installed" is only reported when the read-back matches.

Platform skill directories (data-driven so corrections are one-line):
...
# Hand-enumerated static tuple, never a glob (docs/design.md): later additions
# (unknowns-ledger-v1.schema.json, references/example-session.md, source.md,
# implementation-plan.md, explainer-pitch.md) are one added line each.
MANAGED_FILES = (
    "SKILL.md",
...
def packaged_content(rel: str) -> bytes:
    return files("find_unknowns").joinpath(f"assets/{rel}").read_bytes()
...
def asset_path(rel: str) -> Path | None:
    ...
    """
    traversable = files("find_unknowns").joinpath(f"assets/{rel}")
    try:
